    ...
    async createInternal() {
        ...
        if (!process.env.MODE) {
            for (let index = 0; index < limit; index++) {
                sessions[index] = new Venom(index, `dialogflow${index}.json`, process.env.LANGUAGE_CODE, 'Credencial Interna');
            }
        } else {
            sessions[0] = new Venom(0, `dialogflow0.json`, process.env.LANGUAGE_CODE, 'Credencial Interna');
        }
    },
